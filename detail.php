...
<head>
    ...
    <meta name="description" content="Product overview page of Vinyl Paradise Music Shop">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://cdn.tailwindcss.com"></script>
    <title>Vinyl Planet - Product overview</title>
</head>
<body>

    <?php
    require("configs/db_credentials.php");
    require("functions/connect_to_db.php");
    //connexion
    $con = connect_db($host, $username, $userpassword, $port, $db_name); 
    
    //je récupère les données via l'id mis en url
    
    $id = $_GET["id"];
    $req = $con->prepare("SELECT * from products WHERE id = ? ");
    $req->execute([$id]);
    $res = $req->fetchall(PDO::FETCH_ASSOC);
    $visit = $res[0]["visit"];
    $visit +=1;
    $req = $con->prepare("UPDATE products SET visit = $visit WHERE id = ? ");
    $req->execute([$id]);
...
<section>
      <div class="relative mx-auto max-w-7xl place-content-center px-4 py-8">
        <div>
          <h1 class="text-2xl font-bold lg:text-3xl"><?php echo $res[0]["name"]; ?></h1>

          <p class="mt-1 text-medium text-gray-800"><?php echo $res[0]["artist"]; ?></p>
        </div>
        ...
        <div class="grid gap-8 lg:grid-cols-6 lg:items-center place-content-center">
          ...
          <div class="">
            ...
            >
              ...
              <?php echo $res[0]["description"]; ?><br><br>
              </p>
                <?php
                    $req2 = $con->prepare("SELECT name from categories WHERE id = ? ");
                    $req2->execute([$res[0]["category_id"]]);
                    $res2 = $req2->fetchall(PDO::FETCH_ASSOC);
                ?>
                <form method="POST" action="index2.php" >                 
                  <input type="text" value="on" name="<?php echo $res[0]["category_id"];?>" hidden><button type="submit" class="font-bold">
            ...
            <!-- <form class="space-y-4 lg:pt-8" value="Retour" onclick="history.go(-1)">
              <button
                type="submit"
                class="rounded bg-gray-400 px-6 py-3 text-sm font-bold uppercase tracking-wide "
              >
                Return to shop
              </button>
            </form> -->
            <a class="grid grid-cols-6 gap-4 pt-8" href="index.php">
